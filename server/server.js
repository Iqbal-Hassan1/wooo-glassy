...
process.title = 'Wooo Meeting Server';
...
const interactiveServer = require('./src/interactiveServer');
const interactiveClient = require('./src/interactiveClient');
...
async function createExpressApp() {
    ...
    App.get("/", (req, res, next) => {
        res.send(`server is started at Domain: http://${config.domain}:${config.https.listenPort} and IP Address: ${config.https.listenIp}`)
    })
...
async function runHttpsServer() {
    ...
    await new Promise((resolve) => {
        httpsServer.listen(
            Number(config.https.listenPort), config.https.listenIp, resolve, () => { console.log(`Server started at: https://${config.domain}:${config.https.listenPort}`) });
    });
}
